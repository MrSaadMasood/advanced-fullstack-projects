...
import getFilteredData, {
  filterChatData,
  indexfinder,
  replaceArrayMemberWithModifiedOne,
} from "../utils/filterArrayFunction";
...
export default function Home() {
  ...
  const [messageToDeleteInfo, setMessageToDeleteInfo] =
    useState<MessageToDelete>();
  ...
  const { mutate: deleteMessageMutation } = useMutation({
    ...
    onSuccess: () => {
      ...
      socket.emit(
        "delete-message",
        joinedRoom,
        messageToDeleteInfo.messageId,
        messageToDeleteInfo.type,
        chatId,
      );
      removeDeletedMessageFromChat(
        messageToDeleteInfo.messageId,
  ...
  const filteredNormalChats =
    normalChatData.chat.length === 0
      ? normalChatData
      : (filterChatData(normalChatData, "normal", chatSearchInput) as ChatData);

  const filteredGroupChat =
    groupChatData.length === 0
      ? groupChatData
      : (filterChatData(
          groupChatData,
          "group",
  ...
  useEffect(() => {
    ...
    if (profilePicture) {
      if (isGoogleUser && profilePicture.startsWith("https"))
        return setProfilePictureUrl(profilePicture);
      (async () => {
        const pictureUrl = await fetchPictureFromServer(
          axiosPrivate,
          `/user/get-profile-picture/${userDataFromServer.profilePicture}`,
        );
  ...
  function changeUserDataBasedOnGroupChanges(id: string, actionType: number) {
    const groupToChange = userData?.groupChats.find(
      (group) => group.collectionId === groupManager,
    );
    ...
    setUserData((prevData) => {
      ...
      const groupChatsArrayModified = replaceArrayMemberWithModifiedOne(
        prevData.groupChats,
        groupToChange,
      );
      return {
  ...
  function handleMessageDelete(messageId: string, type: ChatType) {
    setMessageToDeleteInfo({
      collectionId:
        type === "normal" ? normalChatData._id : groupChatData[0]._id,
      type: type,
      messageId,
    });
    ...
    <main>
      ...
      <div>
        ...
        <div className="lg:flex">
          ...
          {optionsSelected !== 6 && (
            <div
              className={`${display} lg:inline h-screen w-full lg:ml-16 lg:w-[23rem]  bg-black lg:border-r-2
            ...
            >
              ...
              <div className="bg-[#1b1b1b] w-full lg:w-[22rem] h-[87vh] overflow-y-scroll noScroll">
                ...
                {optionsSelected === 4 &&
                  userData &&
                  filteredGroupChatList.map((groupChat, index) => {
                    return (
                      <GroupMessagesList
                        chatFriendImageSetter={chatFriendImageSetter}
                        getChatData={getChatData}
                ...
                {optionsSelected === 2 &&
                  filteredFriendsList.map((data, index) => {
                    return (
                      <Friends
                        key={index}
                        data={data}
                ...
                {optionsSelected === 3 &&
                  filteredFollowRequests.map((data, index) => {
                    return (
                      <FriendRequests
                        key={index}
                        data={data}
                ...
                {optionsSelected === 5 &&
                  ...
                  filteredUsers.map((data, index) => {
                    if (
                      ...
                      userData.friends.includes(data._id) === false
                    ) {
                      return (
                        <Users
                          key={index}
