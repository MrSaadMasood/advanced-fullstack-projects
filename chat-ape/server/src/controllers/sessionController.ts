...
} from "../../src/utils/googleTokenFuncs.js";
...
export const createUser = async (req: CustomRequest, res: Response) => {
    ...
    const { fullName, email, password, isGoogleUser, profilePicture, id } =
        req.body;
    ...
    if (!createdUser.acknowledged)
        throw new BadRequest(
            generalErrorMessage("failed to create a new user"),
        );
...
export const loginUser = async (req: CustomRequest, res: Response) => {
    ...
    const userFromDatabase = await database
        .collection<CreateNewUser>("users")
        .findOne({ email: email });
    ...
    if (user.is2FactorAuthEnabled) {
        const factor2AuthToken = sign({ email: email }, F2A_SECRET, {
            expiresIn: "",
        });
        return res.json({
            factor2AuthToken,
...
export const refreshUser = async (req: CustomRequest, res: Response) => {
    ...
    const tokenCheck = await database
        .collection("tokens")
        .findOne({ token: decodedRefreshToken });
    ...
    const newAccessToken = sign({ id: data.id }, ACCESS_SECRET, {
        expiresIn: "5m",
    });
    if (!newAccessToken)
...
export const logoutUser = async (req: CustomRequest, res: Response) => {
    ...
    // const database = await dataBaseConnectionMaker(process.env.TEST_URI || "" )
    const deleteToken = await database
        .collection("tokens")
    ...
    if (deleteToken.deletedCount) {
        ...
        throw new BadRequest(
            generalErrorMessage("faild to delete the refresh token"),
        );
    }
};
...
export const googleAuthenticator = async (
    ...
    const ifUserExists = await database
        .collection("users")
        .findOne<CreateNewUser>({ email: email });
    ...
    if (!ifUserExists) {
        ...
        await database
            .collection("tokens")
            .insertOne({ token: tokens.refresh_token });
