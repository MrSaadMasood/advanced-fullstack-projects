...
import { connectData, getData } from "../../Db/connection.js";
...
export const getUpdatedData = async (req: CustomRequest, res: Response) => {
    ...
    const userFromDatabase = await database
        .collection<DocumentInput>("users")
        .findOne({ _id: id });
...
export const getFollowRequests = async (req: CustomRequest, res: Response) => {
    const { id } = req.user!;
    // const database = await dataBaseConnectionMaker(process.env.TEST_URI || "")
    const receivedRequests = await getCustomData(
        database,
        id,
        "receivedRequests",
    );
...
export const addFriend = async (req: CustomRequest, res: Response) => {
    ...
    const isFriendsListCached = await redisClient.exists(
        `user:friendList:${id}`,
    );
...
export const updateChatData = async (req: CustomRequest, res: Response) => {
    ...
    incomingDataValidationHandler(req);
    // const database = await dataBaseConnectionMaker(process.env.TEST_URI || "")
    const randomMessageId = await updateNormalChatData(
        database,
        collectionId,
        id,
        "content",
...
export const getChatData = async (req: CustomRequest, res: Response) => {
    ...
    const chatData = await database
        .collection<DocumentInput>("normalChats")
        .aggregate([
            {
                $match: {
                    _id: collectionId,
...
export const saveProfilePicturePath = async (
    req: CustomRequest,
    res: Response,
) => {
    const { id } = req.user!;
    const filename = fileValidator(req.file);
    // const database = await dataBaseConnectionMaker(process.env.TEST_URI || "")
    const addingProfilePicture = await database
        .collection<DocumentInput>("users")
        .updateOne(
            { _id: id },
            {
    ...
    if (!addingProfilePicture.modifiedCount)
        throw new BadRequest(
            generalErrorMessage(
                "failed to save the profile picture of the user",
            ),
        );
...
export const getProfilePicture = (req: CustomRequest, res: Response) => {
    ...
    const filepath = path.join(
        currentWorkingDirectory,
        `./src/uploads/profile-images/${name}`,
    );
    res.sendFile(filepath);
};

// deletes the previous profile picture of the user
export const deletePrevProfilePicture = async (
    req: CustomRequest,
    res: Response,
) => {
    const { name } = req.params;
...
export const getGroupChats = async (req: CustomRequest, res: Response) => {
    const { id } = req.user!;
    // const database = await dataBaseConnectionMaker(process.env.TEST_URI || "")
    const groupChats = await database
        .collection<DocumentInput>("users")
        .aggregate([
            {
                $match: {
...
export const filterChat = async (req: CustomRequest, res: Response) => {
    ...
    if (chatType === "group") {
        await database
            .collection<DocumentInput>("groupChats")
            .find({ _id: collectionId })
            .toArray();

        const groupChatData = await database
            .collection<DocumentInput>("groupChats")
            .aggregate([
                {
                    $match: {
                ...
                },
                {
                    $match: {
                        ...
                        $or: [
                            ...
                            {
                                "chat.userId": {
                                    $exists: groupMemberId ? false : true,
                                },
                            },
                        ],
                    },
...
export const updateGroupChatData = async (
    req: CustomRequest,
    res: Response,
) => {
